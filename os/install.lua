...
local function diskinstall()
    ...
    if fs.exists("disk/") then
        ...
        hFile.close()
        for _, file in pairs(fs.list("disk/os/users")) do
            fs.delete("disk/os/users/"..file)
        end
        for _, file in pairs(fs.list("disk/os/programs/notestxt")) do
            fs.delete("disk/os/programs/notestxt/"..file)
        end
        local hFile = fs.open("disk/os/users/root.lua", "w")
        hFile.write('vzMfrxL6+Ujty|N7[>^?_@0($')
        hFile.close()
        local hFile = fs.open("disk/os/users/guest.lua", "w")
